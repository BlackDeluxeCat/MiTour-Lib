...
public class SettingHandler{
    ...
    public static void registerJsonClass(Class<?> clazz){
        Json json = RefUtils.getValue(Core.settings, "json");
        if(json != null){
            ...
            table.table(t -> {
                ...
                UIUtils.tooltip(tb, tooltip -> {
                    tooltip.table(t -> {
                        ...
                        t.table(warning -> {
                            if(restart)
                                warning.left().label(() -> "[orange]" + bundle.get("settings.tags.restart") + "[]").pad(2).padRight(8f);
                            if(reloadWorld)
                                warning.left().label(() -> "[sky]" + bundle.get("settings.tags.reloadWorld") + "[]").pad(2).padRight(8f);
                            if(performance)
                                warning.left().label(() -> "[negstat]" + bundle.get("settings.tags.performance") + "[]").pad(2).padRight(8f);
                        }).growX().left().bottom();
                    }).growX().maxWidth(500f);
                });
            }).size(40f).pad(2f);
    ...
    public class TextFieldSetting extends Setting{
        ...
        @Nullable
        TextField.TextFieldValidator validator;
        @Nullable
